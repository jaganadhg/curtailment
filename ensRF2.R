...
for (i in 1:numBds){
  ...
  for (j in 1:16){
    ...
    df = data.frame(preds2 = predsWS[,j],
                    preds3 = predsKNN[,j],
                       obs = allObs[,j])
    myRF = randomForest(obs ~ preds2 + preds3, data = df, method = "anova")
    RFmodels[[j]] = myRF
  }
  ...
  for (j in 1:16){
    ...
    preds = predict(RFmodels[[j]], 
                  newdata = data.frame(preds2 = predsWStest[,j],
  ...
  setwd("~/Desktop/curtailment/Predictions/ensRF2-test/")
