...
for (i in 1:numBds){
  ...
  for (j in 1:16){
    ...
    df = data.frame(preds1 = predsHM[,j],
                    ...
                    preds3 = predsKNN[,j],
                       obs = allObs[,j])
    myRT = rpart(obs ~ preds1 + preds2 + preds3, data = df, method = "anova")
    RTmodels[[j]] = myRT
  }
